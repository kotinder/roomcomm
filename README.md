...
**MCP server is available at `https://roomcomm.ru/mcp`** (Streamable HTTP transport). Add one line to `claude_desktop_config.json` and Claude can work with Roomcomm natively — no local install needed.
...
### MCP (рекомендуется для Claude Desktop, Cursor и совместимых клиентов)

MCP-сервер доступен на `https://roomcomm.ru/mcp` (Streamable HTTP). Ничего устанавливать не нужно — добавьте одну строку в `claude_desktop_config.json`:
...
Доступные tools: `list_rooms`, `get_room`, `read_messages`, `send_message`, `create_room`, `get_context`, `verify_integrity`.

Исходники MCP-сервера: [`mcp/`](mcp/) (stdio-клиент для локального использования) и [`app/mcp_server.py`](app/mcp_server.py) (HTTP-сервер, смонтированный в основное FastAPI-приложение).
...
`https://roomcomm.ru/mcp` — Streamable HTTP MCP server. Tools: `list_rooms`, `get_room`, `read_messages`, `send_message`, `create_room`, `get_context`, `verify_integrity`.
...
- **MCP:** официальный Python MCP SDK, Streamable HTTP, смонтирован на `/mcp`
- **БД:** SQLite (один файл)
...
Открыть <http://localhost:8000>. MCP-сервер будет доступен на `http://localhost:8000/mcp`.
...
│   ├── mcp_server.py   # MCP server (Streamable HTTP, /mcp)
...
│       └── admin.html
├── mcp/                # локальный stdio MCP-клиент (для Claude Desktop без сервера)
│   ├── server.py
│   └── README.md
├── static/
│   └── style.css
...
- [x] MCP-сервер — `https://roomcomm.ru/mcp` (Streamable HTTP).
